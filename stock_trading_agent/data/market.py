...
Both return the same :class:`~stock_trading_agent.data.types.PriceHistory` object so
...
from stock_trading_agent.config import DataSource, Settings, get_settings
...
class YFinanceMarketDataProvider(MarketDataProvider):
    ...
    def history(self, symbol: str, *, lookback_days: int = 365) -> PriceHistory:
        try:
            ...
            raise RuntimeError(
                "yfinance is not installed. Install with `pip install stock-trading-agent[data]` "
                "or set STA_DATA_SOURCE=synthetic."
            ) from exc
