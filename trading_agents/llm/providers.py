...
class OpenAIChatModel(ChatModel):
    def __init__(self, model: str, api_key: str | None, temperature: float, max_tokens: int):
        try:
            ...
            raise RuntimeError(
                "openai is not installed. `pip install trading-agents[openai]` "
                "or set TA_LLM_PROVIDER=offline."
            ) from exc
...
class AnthropicChatModel(ChatModel):
    def __init__(self, model: str, api_key: str | None, temperature: float, max_tokens: int):
        try:
            ...
            raise RuntimeError(
                "anthropic is not installed. `pip install trading-agents[anthropic]` "
                "or set TA_LLM_PROVIDER=offline."
            ) from exc
