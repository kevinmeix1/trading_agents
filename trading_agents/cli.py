"""Command-line interface for trading_agents.

Examples
--------
    trading-agents analyze AAPL
    trading-agents analyze TSLA --news "beats earnings" --news "analyst upgrade"
    trading-agents backtest NVDA --days 500 --compare
...
from trading_agents.config import DataSource, LLMProvider, Settings, get_settings
...
def config() -> None:
    ...
    s = get_settings()
    table = Table(title="trading_agents configuration")
...
def version() -> None:
    ...
    console.print(f"trading_agents {__version__}")
